...
function requestPost(requestUrl, paramData, successFun, failFun, completeFun, that, isShowLoading, isAddToken) {
  ...
  if (isAddToken) {
    ...
    // paramData.USERNAME = userInfo.Phone;
    // paramData.PASSWORD = userInfo.employeePwd;
...
function pollingRefreshToken() {
  if (app.globalData.userInfo == null) {
    return;
  } else if (app.globalData.userInfo.accessToken == null) {
    return;
  }

  var that = this;
  setTimeout(() => {
    let token = app.globalData.userInfo.accessToken;
    if (token) {
      var paramData = {
        accessToken: token,
      };
      ...
      that.smallRequestPost(config.config.refreshTokenUrl, paramData, function (res) {
        if (res.data.status == 200) {
          // success
          app.globalData.userInfo.accessToken = res.data.body.accessToken;
          app.globalData.userInfo.expiresIn = res.data.body.expiresIn;
...
function doUploadImage(filePath, businessType, fileSize, city, address, success, fail) {
  ...
  var url = this.host_file + config.fileUpload;

  var time = formatAllTime(new Date());
