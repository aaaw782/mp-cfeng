...
Component({
  ...
  methods: {
    ...
    // 输入关键字
    keySearch(event) {
      console.log("输入为："+event.detail.value);

      let { listData } = this.properties;
      let { keyWordsOfShow } = this.data;
      let tmpListData = listData.map((v, i) => v.filter((vv) => {
        return vv[keyWordsOfShow].indexOf(event.detail.value) > -1;
      }));

      this.setData({
        columnsData: tmpListData,
      })
    },
    ...
    sure () {
      ...
      let { listData, keyWordsOfShow } = this.properties;
      let arr = [];
      this._getIndexByIdOfObject(listData[0], backData, keyWordsOfShow, arr);
